...
export const ModalProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [currentModal, setCurrentModal] = 
    useState<React.ReactElement | undefined>(undefined);
  ...
  return (
    <ModalContext.Provider value={{ showModal, currentModal, setModal, removeModal }}>
      {children}
...
export const UrlProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addUrl = useCallback(
    async (
      ...
      const result = await createShortURL(longURL, password, expiration, length, message, customAddress);
      if (result && typeof result === "object" && !("error" in result)) {
        let typedresult = result as {short: string, long: string, usage: number, password: string}
        const newUrl: URL = {
  ...
  const updateUrl = useCallback((shortUrl: string, newUrlShort: string) => {
    setUrls((prev) =>
      prev.map((u) => (u.short === shortUrl ? { ...u, short: newUrlShort } : u))
    );
  }, []);
